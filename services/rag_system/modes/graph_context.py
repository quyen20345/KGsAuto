...
from services.rag_system.modes.common import base_response, effective_budgets, empty_evidence, response_metadata, run_async
...
def run_naive_grag(pipeline, question: str, top_k: Optional[int], include_evidence: bool) -> dict:
    ...
    budgets = effective_budgets(pipeline.config, "naive_grag", top_k)
    return naive_grag_response(question, result, include_evidence, budgets)
...
async def _run_naive_grag_reasoning(pipeline, question: str, top_k: Optional[int]) -> dict:
    graph_top_k = top_k if top_k is not None else pipeline.config.top_k_graph
    return await graph_search_pipeline.naive_grag_reasoning(question, Neo4jAdapter(top_k=graph_top_k))


def naive_grag_response(question: str, result: dict, include_evidence: bool, budgets: dict[str, object]) -> dict:
    evidence = empty_evidence()
    evidence["graph_context"] = result.get("context") or result.get("evidence")

    retrieved_evidence = result.get("retrieved_evidence")
    retrieved_graph_count = len(retrieved_evidence) if isinstance(retrieved_evidence, list) else None
