...
def test_e2e_pipeline_mock_data(tmp_path: Path) -> None:
    ...
    assert s1.indexed_nodes >= 1  # At least 1 node indexed
    ...
    assert "canonical_entities_total" in audit

    with open(s3.id_remap_path, "r", encoding="utf-8") as f:
        actual = json.load(f)

    # Mock data may or may not have duplicates, just verify structure
    assert isinstance(actual, dict), "id_remap should be a dictionary"
