...
from apps.graph_api.schemas import QueryModel
...
def get_random_triplets(limit: int = 10):
    ...
    RETURN coalesce(source_props.id, elementId(n)) AS source_id,
           coalesce(source_props.name, source_props.id, elementId(n)) AS source_name,
           labels(n)[0] AS source_label,
           type(r) AS rel_type, properties(r) AS rel_properties,
           coalesce(target_props.id, elementId(m)) AS target_id,
           coalesce(target_props.name, target_props.id, elementId(m)) AS target_name,
