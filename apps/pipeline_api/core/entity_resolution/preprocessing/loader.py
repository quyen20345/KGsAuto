...
def load_existing_entities_from_neo4j(driver, node_ids: list[str]) -> list[NodeRecord]:
...
def extract_relationship_records(kg_files: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    relationships: list[dict[str, Any]] = []
    for source_file, data in kg_files.items():
        for rel in data.get("relationships", []) or []:
            start = rel.get("source") or rel.get("start_id") or rel.get("start") or rel.get("from")
            end = rel.get("target") or rel.get("end_id") or rel.get("end") or rel.get("to")
