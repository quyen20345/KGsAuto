...
class EntityDB:
    ...
    def __init__(
        ...
        encode_model:    str = "intfloat/multilingual-e5-base",
        device:          str = "cpu",
    ):
        self.collection_name = collection_name
        self.encode_model    = SentenceTransformer(encode_model, device=device)
        self.qdrant_client   = QdrantClient("http://localhost:6333", timeout=300)  # 5 minutes timeout
    ...
    # Write
    def upsert_entities(self, entities: list[dict], batch_size: int = 200) -> int:
        """
        Upsert entities into Qdrant in batches to avoid timeout.
        ...
        # Batch encode for 5-10x speedup
        texts = [self._build_text_for_embedding(e) for e in entities]
